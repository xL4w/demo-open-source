...
To clean up the workflow runs using the below command=
...
After clean-up is performed there should be only one run per workflow in the dashboard.  This allows us to effectively review only the last run of each workflow, to evaluate the current progress, and identify when we can stop to clean up the workflow runs. 
...
How to Test:
- Create a separate branch called "test-branch" for testing.
- Commit and push the workflows, scripts, and branch protection rules.
- Trigger the test workflows either locally or through your CI/CD pipeline.
